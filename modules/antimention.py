...
class AntiMentionMod(loader.Module):
    ...
    async def antimentioncmd(self, message):
        ...
        if args == "clearall":
            ...
            return await message.edit("<b>Режим выключен во всех чатах</b>")
        if not message.is_private:
            ...
            if chatid in am:
                ...
                return await message.edit("<b>Деактивирован в этом чате</b>")
            ...
            self.db.set("AntiMention", "action", ac)
            self.db.set("AntiMention", "am", am)
            return await message.edit("<b>Активирован в этом чате</b>")
        else: return await message.edit("<b>Это не чат</b>")
    ...
    async def amexcmd(self, message):
        ...
        if not message.is_private:
            ...
            if chatid not in am: return await message.edit("<b>В этом чате режим деактивирован</b>")
            ...
            if args == "clear":
                ...
                self.db.set("AntiMention", "action", ac)
                return await message.edit(f"<b>Список исключений в этом чате очищен</b>")
            try:
            ...
            self.db.set("AntiMention", "action", ac)
            return await message.edit(f"<b>{user.first_name} удалён из списка исключений</b>")
        else: return await message.edit("<b>Это не чат</b>")
        ...
        if not message.is_private:
            ...
            if chatid in am:
                if args:
                    ...
                    else: return await message.edit("<b>Такого режима нет в списке\nДоступные режимы: kick/ban/mute/none</b>")
                    self.db.set("AntiMention", "action", ac)
                    return await message.edit(f"<b>Теперь, при упоминании в чате будет выполняться действие: {ac[chatid]['action']}</b>")
                else: return await message.edit(f"<b>Настройки чата:</b>\n\n:"
                                                f"<b>Состояние режима:</b> True\n"
                                                f"<b>При упоминании в чате будет выполняться действие:</b> {ac[chatid]['action']}\n")
            else: return await message.edit("<b>В этом чате режим деактивирован</b>")
        else: return await message.edit("<b>Это не чат</b>")
